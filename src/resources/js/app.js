...
new Vue({
  ...
  created() {
    ...
    userApi.getPlayer(res => { users.players = res });
    userApi.getStaff(res => { users.staff = res });
    userApi.getAlumni(res => { users.alumni = res });
    userApi.getOfficer(res => { users.officers = res });
    userApi.getActiveOb(res => { users.activeAlumni = res });

    // set twitter API response
    const twitter = this.twitter;
    const timeline = twitter.timeline;
    twitterApi.getProvider(res => { twitter.providers = res });
    twitterApi.getTimeline(res => {
      timeline.tweets = res;
